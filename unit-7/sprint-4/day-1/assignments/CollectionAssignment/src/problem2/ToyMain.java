...
public class ToyMain {
    ...
    public static void main(String[] args) {
        //Create object of problem2.ToyOperation class
        ToyOperation toyOperations = new ToyOperation();
//        System.out.println(problem2.CompanyCodeName.companyCode.size());
        try {
        ...
        try {
            ...
            toyOperations.addToy("NKT008", "Snack-Ladder", 399.00, "Wooden problem2.Toy");
        ...
        try{
            ...
            System.out.println("problem2.Toy Number\tproblem2.Toy Name\t\tPrice\t\tCategory");
            for(String toy: list)
...
//            toy = toyOperations.getToyByToyNumber("NKT010");
        ...
        System.out.println("-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*");
        List<String> list1 = toyOperations.getToyListSortedByPrice();
        System.out.println("problem2.Toy Number\tproblem2.Toy Name\t\tPrice\t\tCategory");
        for(String toy: list1)
            System.out.println(toy);
        System.out.println("-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*");
        try{
            List<String> list =
                    toyOperations.getToyListForGivenPriceRange(599.00, 1299.00);
            System.out.println("problem2.Toy Number\tproblem2.Toy Name\t\tPrice\t\tCategory");
            for(String toy: list)
                System.out.println(toy);
            System.out.println("-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*");
//            list = toyOperations.getToyListForGivenPriceRange(1300.00,
//                    1500.00);
        }catch(NoToyException ex) {
            System.out.println(ex.getMessage());
        }
